...
class FormulaTransform(Scene):
    def construct(self):
        divider = Line(start=(-10, -1, 0), end=(10, -1, 0))
        self.play(Write(divider))
        formula1 = MathTex(r"{{f(x)}} = {{a}}{{x^2}} + {{b}}{{x}} + {{c}}").shift(UP * 1)
        formula1_copy = formula1.copy()
        self.play(Write(formula1))
        self.play(formula1_copy.animate.shift(DOWN * 3))
        self.play(Circumscribe(formula1[5]))
        formula2 = MathTex(r"{{f(x)}} = {{a}}{{x^2}} + {{\frac{a \cdot  b}{a}}}{{x}} + {{c}}").shift(UP * 1)
        self.play(TransformMatchingTex(formula1, formula2))
        self.pause(1)
        self.play(Indicate(formula2[2]), Indicate(formula2[5][0]))
        self.pause(1)
        formula3 = MathTex(r"{{f(x)}} = {{a}}\left({{x^2}} + {{\frac{b}{a}}}{{x}}\right) + {{c}}").shift(UP * 1)
        self.play(TransformMatchingTex(formula2, formula3))
        self.pause(1)
        formula3_copy = formula3.copy()
        formula3_copy.remove(formula3_copy[0])
        self.play(formula1_copy.animate.shift(LEFT * 2))
        self.play(formula3_copy.animate.next_to(formula1_copy))
        self.pause(1)
        formula4 = MathTex(r"{{f(x)}} = {{a}}\left({{x^2}} + 2 \cdot {{\frac{b}{2a}}}{{x}}\right) + {{c}}").shift(UP)
        ...
        self.pause(1)
        formula5_copy = formula5.copy()
        self.play(FadeOut(formula1_copy, formula3_copy), formula5_copy.animate.shift(DOWN * 3))
        self.pause(1)
        self.play(Circumscribe(formula5[12]))
        formula6 = MathTex(r"{{f(x)}} = {{a}}\left({{x^2}} + 2 \cdot {{\frac{b}{2a}}}{{x}} + "
                           r"{{\left(\frac{b}{2a}\right)^2}} - {{\frac{b^2}{4a^2}}} \right) + {{c}}").shift(UP)
        self.play(TransformMatchingTex(formula5, formula6))
        formula7 = MathTex(r"{{f(x)}} = {{a}}\left({{x^2}} + 2 \cdot {{\frac{b}{2a}}}{{x}} + "
                           r"{{\left(\frac{b}{2a}\right)^2}} \right) - {{\frac{b^2}{4a^2}}} + {{c}}").shift(UP)
        ...
        self.pause(1)
        formula8 = MathTex(r"{{f(x)}} = {{a}}\left({{x}} + {{\frac{b}{2a}}}\right)^2 - {{\frac{b^2}{4a^2}}} + {{c}}").shift(UP)
        self.play(TransformMatchingTex(formula7, formula8), FadeOut(hint_f7))
        self.pause(1)
        formula8_copy = formula8.copy()
        self.play(FadeOut(formula5_copy), formula8_copy.animate.shift(DOWN * 3))
        self.pause(1)
        self.play(Circumscribe(formula8[9]))
        formula9 = MathTex(r"{{f(x)}} = {{a}}\left({{x}} + {{\frac{b}{2a}}}\right)^2 - {{\frac{b^2 - 4ac}{4a}}}").shift(UP)
        self.play(TransformMatchingTex(formula8, formula9))
        self.pause(1)
        self.play(Circumscribe(formula9[8]))
        final_formula = MathTex(r"{{f(x)}} = {{a}}\left({{x}} + {{\frac{b}{2a}}}\right)^2 {{- \frac{D}{4a}}}").shift(UP)
        self.play(TransformMatchingTex(formula9, final_formula))
        self.pause(1)
        self.play(FadeOut(divider, formula8_copy), final_formula.animate.shift(DOWN * 1))
        self.pause(1)
